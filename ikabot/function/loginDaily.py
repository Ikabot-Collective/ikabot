    ...
    while True:
        ...
        if wood_city:
            ...
            if 'js_nextPossibleResource' in features[0]:
                earliest_wakeup_time = get_remaining_time_cinetheatre(features) if get_remaining_time_cinetheatre(features) < earliest_wakeup_time else earliest_wakeup_time 
            else:
        ...
        if wood_city or luxury_city or favour_cinetheater_city:
            html = session.post(city_url + str(wine_city['id']))
            html = session.post(f"view=cinema&visit=1&currentCityId={wine_city['id']}&backgroundView=city&actionRequest={actionRequest}&ajax=1")
            features = html.split('id=\\"VideoRewards\\"')[1].split('ul>')[0].split('li>')
            features = [f for f in features if 'form' in f or 'js_nextPossible' in f]
            earliest_wakeup_time = get_remaining_time_cinetheatre(features) if get_remaining_time_cinetheatre(features) < earliest_wakeup_time else earliest_wakeup_time
...
def is_collectable(row):
    ...
    progress_right = re.search(r'left small progress details([\S\s]*?)>([\S\s]*?)<', row).group(2).strip().replace(',','')
...
def stay_online_30_mins(session, table):
    ...
    for r in table:
        if 'task_amount_23' in r: # stay online for 30 mins
            if is_collectable(r):
                session.post(f"action=CollectDailyTasksFavor&taskId=23&ajax=1&backgroundView=city&currentCityId={wine_city['id']}&templateView=dailyTasks&actionRequest={actionRequest}&ajax=1")
                wait(1)
            else:
