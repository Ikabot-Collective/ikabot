...
MilitaryAndFleetMovements = TypedDict("MilitaryAndFleetMovements", {...}) # TODO figure out what this dict looks like?
AlertLowWineConfig = TypedDict("AlertLowWineConfig", {"hours": int, "auto_transfer": bool, "transfer_amount": int})
def alertLowWine(session: Session) -> AlertLowWineConfig:
    ...
    return {"hours": hours, "auto_transfer": auto_transfer, "transfer_amount": transfer_amount}

def do_it(session: Session, hours: int, auto_transfer: bool, transfer_amount: int):
    was_alerted = {}
    message_log = []
...
def getMovementsFromHtml(session: Session) -> list[MilitaryAndFleetMovements]:
...
def isWineTransportInProgress(session: Session, destinationCityId: int) -> Union[str, None]:
