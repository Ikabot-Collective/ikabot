...
SellResourcesConfig = Union[TypedDict("SellResourcesConfig", {"left_to_sell": int, "offers": list[tuple[str, str, int, int, str, int]], "resource_type": int, "city_to_buy_from": FullCityDict}) , TypedDict("SellResourcesConfig", {"amount_to_sell": int, "price": int, "resource_type": int, "sell_market_capacity": int, "city": FullCityDict})]
def sellResources(session: Session) -> SellResourcesConfig:
    banner()
    ...
    return [sellToOffers, createOffer][selected - 1](session, city, resource)
...
def sellToOffers(session, city_to_buy_from, resource_type):
    ...
    return {"type": "existing", "amount_to_sell": amount_to_sell, "chosen_offers": chosen_offers, "resource_type": resource_type, "city_to_buy_from": city_to_buy_from}
...
def do_it(session: Session, type: str, **kwargs):
    if type == "existing":
        return do_it1(session, **kwargs)
    if type == "create":
        return do_it2(session, **kwargs)

def do_it1(session, left_to_sell: int, offers: list[tuple[str, str, int, int, str, int]], resource_type: int, city_to_buy_from: FullCityDict):
    for offer in offers:
...
def do_it2(session: Session, amount_to_sell: int, price: int, resource_type: int, sell_market_capacity: int, city: FullCityDict):
