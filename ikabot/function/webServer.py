...
def mayorMessageResponse(type: ResponseTypes, message: str):
    return f"""[["provideFeedback",[{{"location":1,"type":{type},"text":"{message}"}}]]]"""
...
def generateTableHTML(process_list):
    ...
    for process in process_list:
        ...
        table_html += f"{tr_string}<td>{process['pid']}</td><td>{process['action']}</td><td>{datetime.fromtimestamp(process['date']).strftime('%Y-%m-%d %H:%M:%S')}</td><td style=\"font-size:0.8em\">{process['status']}</td><td><button class=\"button\" onclick=\"ajaxHandlerCall('?action=killTask&pid={process['pid']}&ikabot=1'); ajaxHandlerCall('?view=ikabotSandbox&activeTab=tab_ikabotSandbox');\">Kill</button></td></tr>"
        i+=1
...
window.onload = function() {{
    ...
    document.querySelector('span[title="Version"]').innerHTML = '{"ikabot v7.0.0"}'; // will be replaced
