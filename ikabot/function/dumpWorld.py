...
def do_it(session, waiting_time, start_id, shallow):
    ...
    update_status(
        "Got {} islands with {} towns in total".format(
            len(all_island_ids), str(total_settlements)
        ),
        100,
        5,
        True,
    ...
    dump_islands(
        ...
        start_id,
        session,
        world_islands_number,
    )

    update_status(
        "Got {} individual islands".format(world_islands_number), 100, 100, True
    )
...
def dump_islands(
    shared_data, all_island_ids, waiting_time, start_id, session, world_islands_number
):
    for i, island_id in enumerate(sorted(map(int, all_island_ids))):
        if int(island_id) < start_id:
            continue
        update_status(
            "Getting island id {}".format(island_id),
    ...
    while True:
        ...
        if choice == 0:
            ...
            inactives = [
                ...
                and isinstance(
                    [
                        (seen.add(city["ownerName"]),)
                        if city["ownerName"] not in seen
                        else None
                    ][0],
                    tuple,
                )
